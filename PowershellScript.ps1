...
$Miro = Read-Host "`nPlease enter the software name."

try 
{
    # Replace occurrences of "Miro" in all files
    Write-Host "`nReplacing occurrences of 'Miro' with "$Miro" in all files.`n"
    ...
    Get-ChildItem -File -Recurse | ForEach-Object {
        ...
        try
        {
            (Get-Content $_.FullName) -replace 'Miro', $Miro | Set-Content $_.FullName
        } 
        catch 
        {
    ...
    Write-Host "`nRenaming files and directories using "$Miro":`n"
    ...
    # Get all files and directories containing "Miro" recursively
    Get-ChildItem -Recurse -Directory | ForEach-Object {
        $dirpath = $_.FullName
        $dirname = Split-Path  $dirpath -Leaf
        if ($dirname.Contains("Miro"))
        {
            $stack.Push($dirpath)
        ...
        foreach ($file in [Directory]::EnumerateFiles($dirpath)) 
        {
            ...
            if ($filename.Contains('Miro') -and -not $allPaths.Contains($file))
            {
                $stack.Push($file)
                $allPaths.Add($file)
            }
    ...
    while ($stack.Count) {
        ...
        if($filename.Contains('Miro') -and $pathExists)
        {
            $newName = $filename.Replace('Miro', $Miro)
